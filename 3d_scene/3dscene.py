...
# DOPE configuration
DOPE_ENABLED = True  # Enable DOPE for tool and case 6D pose detection (required for screw tracking)
...
DOPE_USE_FP16 = True  # Use half-precision for faster GPU inference
DOPE_STOP_AT_STAGE = 1  # Stop at stage 1 for maximum speed (trade-off with accuracy)
...
class SyncedVideoManager:
    ...
    def __init__(self):
        ...
        self.yolo_inference_interval = 15  # Run YOLO every 15 frames (skip for speed)
    ...
    def advance_frame(self):
        ...
        if run_dope and len(self.dope_objects_by_camera) > 1:
            ...
            for camera_id in self.camera_ids_list:
                camera_id, jpeg_data, dope_results, ended, raw_frame = (
                    self._process_camera(camera_id, (run_yolo, run_dope), run_vggt)
                )
                if ended:
                    video_ended = True
